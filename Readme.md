...
You can now modify the style or logic of your storefront.  Any new JS scripts or CSS files need to be added to index.html. The multi-tenant mode also points to index.html, and it is possible to utilize jade server-side templating through index.jade as an alternative.
...
There is some minimal bootstrapping in place that determines the tenant id that's associated with the request for an inbound URL.
