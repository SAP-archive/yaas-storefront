...
Performance optimizations are included in the gruntfile to improve the initial load time of the site. There is an 'optimizeCode' grunt task which concatenates and minifies JavaScript and CSS to reduce HTTP requests and the overall page size of the application. It works by pulling all the code from the development files into the .tmp directory, where it then concatenates and minifies before moving it to a final destination in the /dist directory. The 'optimizeCode' conducts all tasks required with producing an optimized run-time, including: cleaning of /dist, copying of dependencies, and the replacement of the concatenated and minified resources. The code within the /dist directory then contains everything needed to run an optimized store front. To deploy those optimized resources, first run a build command that will populate the /dist directory with the optimized files, like **grunt build** for example, then deploy /dist to your server of choice and run **grunt startServer**. The build task for startServer gives you the choice to run either a multi-project site or a single-project site with the flags --single or --multiple, and they will start the server with only the required build tasks for that distribution and excludes any other unecessary build tasks like linting.
